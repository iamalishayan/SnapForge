...
const server = setupServer(
  ...
  http.post('*/rest/v1/articles', async ({ request }) => {
    return HttpResponse.json({ id: '3', title: 'New Article' }, { status: 201 })
  })
)
...
describe('Articles API (Integration)', () => {
  ...
  it('POST /api/v1/articles validates input via Zod and uses MSW', async () => {
    ...
    const badReq = new Request('http://localhost:3000/api/v1/articles', {
      ...
      body: JSON.stringify({ status: 'draft' }) // Missing required title, slug, template_id
    })
    ...
    const goodReq = new Request('http://localhost:3000/api/v1/articles', {
      ...
      headers: { 
        ...
        'x-template-id': '00000000-0000-0000-0000-000000000000'
      },
      body: JSON.stringify({ 
        title: 'New Article', 
        slug: 'new-article',
