...
import { useTemplate } from "@/lib/hooks/use-data";

export default function EditTemplatePage({ params }: { params: { id: string } }) {
  const router = useRouter();
  const { id } = params;
  const { data, error, isLoading } = useTemplate(id);
  ...
  useEffect(() => {
    if (data?.data) {
      ...
      setFormData({
        name: template.name || "",
        slug: template.slug || "",
        gemini_prompt: template.gemini_prompt || "",
        active: template.active ?? true,
        category: template.category || "Content Marketing",
        meta_title: template.meta_title || "",
        meta_description: template.meta_description || "",
      });
      
      const standardCategories = ["Content Marketing", "SEO Automation", "Social Media", "Code Generation", "Email Campaigns"];
      if (template.category && !standardCategories.includes(template.category)) {
  ...
  if (error || !data?.data) {
    return (
      <div className="flex-1 overflow-y-auto px-8 py-8 flex flex-col items-center justify-center text-center">
        ...
        <p className="text-muted-foreground mb-6">The template you are trying to edit does not exist or was deleted.</p>
        <Link href="/admin/templates" className="px-6 py-2 bg-primary text-primary-foreground rounded-lg text-sm font-semibold hover:brightness-110 transition-all active:scale-95">
          Go Back
        </Link>
      </div>
    );
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-8">
            <div>
              <nav className="flex items-center space-x-2 mb-2 text-muted-foreground text-xs font-medium">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m9 18 6-6-6-6"/></svg>
                <span className="text-primary truncate max-w-[200px] inline-block align-bottom">{formData.name || "Edit"}</span>
              </nav>
              <h1 className="text-3xl font-bold text-foreground tracking-tight">Edit Template</h1>
            </div>
                      ...
                      <input 
                        ...
                        onChange={(e) => {
                          setFormData({
                            ...formData, 
                            name: e.target.value,
                            slug: formData.slug && formData.slug !== handleSlugify(data.data.name) ? formData.slug : handleSlugify(e.target.value)
                          });
                        }}
